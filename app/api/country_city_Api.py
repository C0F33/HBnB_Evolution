...
@country_city_Api_blueprint.route('/countries/<country_code>', methods=['GET'])
...
@country_city_Api_blueprint.route('/countries/<country_code>/cities', methods=['GET'])
def get_cities_by_country(country_code):
	if not data_manager.get(country_code, 'Country'):
		abort(404, description="Country not found")
	cities = [city.to_dict() for city in data_manager.storage.get('City', {}).values() if city.country_code == country_code]
	return jsonify(cities), 200

@country_city_Api_blueprint.route('/cities', methods=['POST'])
def create_city():
	...
	existing_cities = [city for city in data_manager.storage.get('City', {}).values() if city.name == name and city.country_code == country_code]
	if existing_cities:
...
@country_city_Api_blueprint.route('/cities', methods=['GET'])
def get_cities():
	cities = [city.to_dict() for city in data_manager.storage.get('City', {}).values()]
	return jsonify(cities), 200

@country_city_Api_blueprint.route('/cities/<city_id>', methods=['GET'])
...
@country_city_Api_blueprint.route('/cities/<city_id>', methods=['PUT'])
...
@country_city_Api_blueprint.route('/cities/<city_id>', methods=['DELETE'])
